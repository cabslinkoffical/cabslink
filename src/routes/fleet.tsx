...
function ClassRow({ klass, index, reverse }: { klass: PublicVehicleClass; index: number; reverse: boolean }) {
  ...
  return (
    <article
      id={klass.slug}
      className={`grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-center ${reverse ? "md:[&>div:first-child]:order-2" : ""}`}
    >
      {/* IMAGE PLATE */}
      <div className="md:col-span-6">
        <div className="relative rounded-3xl bg-[var(--surface,#f5f2ec)] aspect-[16/10] p-6 md:p-8 flex items-center justify-center overflow-hidden">
          {/* Big faded number */}
          <span
            aria-hidden
            className="absolute -top-4 md:-top-6 left-4 md:left-6 font-display font-semibold leading-none text-[var(--navy)]/[0.06] text-[110px] md:text-[160px] select-none"
          >
          ...
          {img ? (
            <img
              ...
              loading="lazy"
              decoding="async"
              className="relative max-h-full max-w-full w-auto h-auto object-contain drop-shadow-[0_22px_22px_rgba(14,24,44,0.18)] transition-transform duration-500 hover:scale-[1.03]"
            />
          ) : (
      ...
      {/* CONTENT */}
      <div className="md:col-span-6">
        <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[var(--gold)]">Vehicle Class · {String(index + 1).padStart(2, "0")}</p>
        <h2 className="mt-2 font-display text-2xl md:text-3xl xl:text-4xl font-semibold text-foreground leading-tight">
          {klass.name}
        </h2>
        {klass.short_description && (
          <p className="mt-3 text-muted-foreground max-w-xl">{klass.short_description}</p>
        )}

        {/* Specs */}
        <div className="mt-5 grid grid-cols-3 gap-2 max-w-md">
          <MiniSpec icon={<Users className="size-3.5" />} label="Passengers" value={String(klass.passengers)} />
          <MiniSpec icon={<Briefcase className="size-3.5" />} label="Large bags" value={String(klass.large_luggage)} />
          <MiniSpec icon={<Luggage className="size-3.5" />} label="Cabin bags" value={String(klass.cabin_bags)} />
        </div>
        ...
        {recs.length > 0 && (
          <div className="mt-5 flex flex-wrap gap-1.5">
            {recs.map((r) => (
              <span key={r} className="inline-flex items-center gap-1 rounded-full bg-[var(--navy)]/[0.06] text-[var(--navy)] px-2.5 py-1 text-[11px] font-medium">
          ...
          <div className="mt-5">
            ...
            <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
              {klass.models.slice(0, 4).map((m) => (
                <span key={m.id} className="inline-flex items-center gap-1 text-[12px] text-foreground/80">
                  <Star className="size-2.5 fill-[var(--gold)] text-[var(--gold)]" /> {m.name}
                </span>
              ))}
              {klass.models.length > 4 && (
                <span className="text-[12px] text-muted-foreground">+{klass.models.length - 4} more</span>
              )}
            </div>
          </div>
        )}
        ...
        <div className="mt-7 flex flex-wrap items-center gap-2">
          ...
          <Button asChild variant="outline" className="rounded-full">
            <Link to="/contact"><ShieldCheck className="size-3.5" /> Talk to us</Link>
          </Button>
        </div>
      </div>
    </article>
...
function MiniSpec({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border bg-card px-3 py-2.5 text-center">
      ...
      <div className="mt-0.5 font-semibold text-foreground text-base">{value}</div>
    </div>
  );
}
