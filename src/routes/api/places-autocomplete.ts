...
async function lookup(data: z.infer<typeof acInput>): Promise<{ suggestions: PlaceSuggestion[] }> {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const parsed = acInput.safeParse(body);
        if (!parsed.success) return Response.json({ suggestions: [] }, { status: 200 });
