 ...
 * Emits Tier 1 destinations only (indexable, non-noindex).
 */
...
// URL param `type` is the destination_type (e.g. "airport", "location").
export const Route = createFileRoute("/sitemaps/$type.xml")({
  server: {
    handlers: {
      GET: async ({ params }) => {
        const type = params.type as DestinationType;
