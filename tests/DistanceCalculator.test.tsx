...
import type { SelectedPlace } from "@/components/site/PlaceAutocomplete";

vi.mock("@tanstack/react-start", () => ({
  useServerFn: (fn: unknown) => fn,
}));

// Stub PlaceAutocomplete so the test drives DistanceCalculator's own logic.
vi.mock("@/components/site/PlaceAutocomplete", async () => {
  ...
  function PlaceAutocomplete(props: {
    ...
    const testId = props.id === "dc-pickup" ? "pickup" : "dest";
    return React.createElement(
      "div",
      null,
      React.createElement("span", { "data-testid": `${testId}-label` }, props.value?.label ?? ""),
      React.createElement("button", { type: "button", "data-testid": `${testId}-set-a`, onClick: () => props.onChange({ placeId: "PID_A", label: "Location A" }) }, "set A"),
      React.createElement("button", { type: "button", "data-testid": `${testId}-set-b`, onClick: () => props.onChange({ placeId: "PID_B", label: "Location B" }) }, "set B"),
      React.createElement("button", { type: "button", "data-testid": `${testId}-clear`, onClick: () => props.onChange(null) }, "clear"),
    );
  }
